...
export function list(req, res, next) {
  ...
  let _flatten = req.query._flatten
  delete req.query.q
  ...
  if (q) {
    ...
    chain = chain.filter(obj => {
      for (let key in obj) {
        if (attr && attr.indexOf === -1) continue
        const value = obj[key]
  ...
  chain = chain.cloneDeep().forEach(function(element) {
    Utils.embed(db, opts, name, element, _embed)
    Utils.expand(db, opts, element, _expand)
  })

  // flatten nested objects
  if (_flatten !== undefined) {
    let fs = Utils.getFlattenSettings(_flatten)
    res.flatSettings = `allowArrays: ${fs.allowArrays} , flattenArrays: ${fs.flattenArrays}`
    chain = chain.flatMap(x => Utils.flatten(x, fs))
  }
...
export function single(req, res, next) {
  ...
  if (resource) {
    ...
    Utils.embed(db, opts, name, clone, _embed)

    // Expand inner resources based on id
    // /posts/1?_expand=user
    Utils.expand(db, opts, clone, _expand)

    if (_flatten) {
      let fs = Utils.getFlattenSettings(_flatten)
      res.flatSettings = `allowArrays: ${fs.allowArrays} , flattenArrays: ${fs.flattenArrays}`
      clone = Utils.flatten(clone, fs)
    }
